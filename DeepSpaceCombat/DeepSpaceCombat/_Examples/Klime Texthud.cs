            ...
            try
            {
                ...
                if (timer % 10 == 0)
                {
                        ...
                        && MyAPIGateway.Session.CameraController != null && MyAPIGateway.Session.CameraController.IsInFirstPersonView && MyAPIGateway.Session.Camera != null)
                    {
                        ...
                        MyAPIGateway.Physics.CastRay(camera_matrix.Translation, camera_matrix.Translation + camera_matrix.Forward * 8, out raycast);

                        if (raycast != null && raycast.HitEntity != null)
                        {
                            reuse_cubegrid = raycast.HitEntity as MyCubeGrid;
                            if (reuse_cubegrid == null || reuse_cubegrid.Physics == null || reuse_cubegrid.IsStatic || reuse_cubegrid.GridSizeEnum == MyCubeSize.Large)
                            {
                                reuse_cubegrid = null;
                            }
