...
namespace Phoera
{
    ...
    public class Hasher
    {
        ...
        enum MD5InitializerConstant : uint
        {
            A = 0x67452301,
            B = 0xEFCDAB89,
            C = 0x98BADCFE,
        ...
        public ulong GetOrCompute(string value)
        {
            ulong result;
            if (_hashes.TryGetValue(value, out result))
                return result;
            return _hashes[value] = Compute(value);
        }

        public static bool CheckUnique(params string[] strings)
        {
            var hash = new HashSet<ulong>();
            return strings.Distinct().Select(Compute).All(hash.Add);
        }

        static ulong Compute(string value)
        {
            var hash = new MD5();
            hash.Value = value;
            var u1 = (ulong)hash.Digest.A;
            u1 <<= 32;
            u1 |= hash.Digest.B;
            var u2 = (ulong)hash.Digest.C;
            u2 <<= 32;
            u2 |= hash.Digest.D;
            return u1 ^ u2;
        }
