...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        {"3D_Pist_Forward_0","3D_Pist_Forward_1"};
        ...
        public void Main(string argument)
        {
            ...
            if (!d3_init)
            {
                ...
                // Init welder
                drillObject = (IMyShipDrill)GridTerminalSystem.GetBlockWithName(drillName);
                if (null == drillObject)
                {
                    fatal_error("Drill Name not found");
                }
                drillObject.ApplyAction("OnOff_Off");
            ...
            else if (!d3_Status)
            {
                ...
                drillObject.ApplyAction("OnOff_On");
                    ...
                    else // Move forward
                    {
                        foreach (KeyValuePair<int, piston> entry in dPiston)
                        {
                            ...
                            if ((entry.Value.grid.CurrentPosition + step) > entry.Value.maxLength)
                            {
                                ...
                                setPistons(parseNames(entry.Value.name), entry.Value.maxLength, 0.4f, false);
                            }
                            else
                            {
                                setPistons(parseNames(entry.Value.name), step, 0.4f, true);
                            }
                ...
                else if (d3_activeAxis == "z")
                {
                    ...
                    if (!pistLimitCheck("z", false))
                    {
                        foreach (KeyValuePair<int, piston> entry in dPiston)
                        {
                            ...
                            // Set piston 
                            setPistons(parseNames(entry.Value.name), 5f, 0.4f, true);
        ...
        private void pistZero()
        {
            foreach (KeyValuePair<int, piston> entry in dPiston)
            {
                ...
                if (entry.Value.direction)
                {
                    ...
                    {
                        // Set piston to zero
                        setPistons(parseNames(entry.Value.name), 0, 0.4f, false);
                    }
                }
                else
                {
